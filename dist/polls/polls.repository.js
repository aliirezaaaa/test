...
let PollsRepository = PollsRepository_1 = class PollsRepository {
    ...
    async addParticipant({ pollID, userID, name, }) {
        ...
        try {
            await this.redisClient.sendCommand(new ioredis_1.Redis.Command('JSON.SET', [key, participantPath, JSON.stringify(name)]));
            const pollJSON = await this.redisClient.sendCommand(new ioredis_1.Redis.Command('JSON.GET', [key, '.']));
